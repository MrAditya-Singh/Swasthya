...
async function checkSlotConflict({ doctor_id, appointment_date, time_slot, exclude_appointment_id = null }) {
    ...
    // Also check localDb for local/test/fallback mode
    try {
        const localApts = localDb.getCollection('appointments') || [];
        const conflict = localApts.some(a =>
            a &&
...
async function bookAppointment({
    ...
    const formattedSlot = time_slot || (is_walk_in ? 'Walk-in Immediate Queue' : '10:00 AM - 12:00 PM');
...
async function rescheduleAppointment(appointmentId, { new_date, new_slot, reason, user }) {
    ...
    if (!apt) {
        apt = (localDb.getCollection('appointments') || []).find(a => a && a.id === appointmentId);
    }
    ...
    localDb.update('appointments', a => a.id === appointmentId, updatePayload);
    if (!updatedApt) {
        updatedApt = { ...apt, ...updatePayload };
    }
...
async function cancelAppointment(appointmentId, { reason, user }) {
    ...
    localDb.update('appointments', a => a.id === appointmentId, updatePayload);
    if (!updatedApt) {
        updatedApt = apt ? { ...apt, ...updatePayload } : { id: appointmentId, ...updatePayload };
    }
...
async function markMissedAppointment({ appointmentId, referralId, reason, user }) {
    ...
    if (appointmentId) {
        ...
        localDb.update('appointments', a => a.id === appointmentId, { status: 'missed', updated_at: new Date().toISOString() });
    }
